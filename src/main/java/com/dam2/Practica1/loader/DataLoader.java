...
public class DataLoader implements CommandLineRunner {
        ...
        public void run(String... args) throws Exception {
                ...
                createMovie("Catch Me If You Can", 141, 12, directors.get(0),
                                Arrays.asList("Leonardo DiCaprio", "Tom Hanks"),
                                actorMap);
                createMovie("Saving Private Ryan", 169, 16, directors.get(0), Arrays.asList("Tom Hanks", "Matt Damon"),
                                actorMap);

                createMovie("Pulp Fiction", 154, 18, directors.get(1),
                                Arrays.asList("Samuel L. Jackson", "Uma Thurman"),
                                actorMap);
                ...
                createMovie("The Irishman", 209, 16, directors.get(2), Arrays.asList("Robert De Niro", "Al Pacino"),
                                actorMap);

                createMovie("Inception", 148, 12, directors.get(3), Arrays.asList("Leonardo DiCaprio", "Tom Hardy"),
                                actorMap);
                createMovie("The Dark Knight Rises", 164, 12, directors.get(3),
                                Arrays.asList("Christian Bale", "Tom Hardy", "Morgan Freeman"), actorMap);
                createMovie("Interstellar", 169, 12, directors.get(3),
                                Arrays.asList("Matthew McConaughey", "Matt Damon"),
                                actorMap);

                createMovie("Se7en", 127, 18, directors.get(4), Arrays.asList("Brad Pitt", "Morgan Freeman"), actorMap);
                createMovie("Fight Club", 139, 18, directors.get(4), Arrays.asList("Brad Pitt", "Edward Norton"),
                                actorMap);

                createMovie("Ocean's Eleven", 116, 12, directors.get(5), Arrays.asList("Brad Pitt", "Matt Damon"),
                                actorMap);
                createMovie("Contagion", 106, 12, directors.get(5), Arrays.asList("Matt Damon", "Kate Winslet"),
                                actorMap);

                createMovie("The Godfather Part II", 202, 18, directors.get(6),
                                Arrays.asList("Al Pacino", "Robert De Niro"),
                                actorMap);

                createMovie("Star Wars: A New Hope", 121, 0, directors.get(7),
                                Arrays.asList("Harrison Ford", "Mark Hamill"),
                                actorMap);

                createMovie("Body of Lies", 128, 16, directors.get(8),
                                Arrays.asList("Leonardo DiCaprio", "Russell Crowe"),
                                actorMap);
